...
async function updateUI() {
  try {
    ...
    $("#wifiProgressBar").width(
      (signalPercentage * wifiNozzleParentWidth) / 100
    );

    if (signalPercentage > 85) {
      $("#wifiProgressBar").css("background-color", "#51a34f");
    } else if (signalPercentage > 70) {
      $("#wifiProgressBar").css("background-color", "darkgreen");
...
setInterval(async () => {
  try {
    var telemetryObject = await updateUI();
